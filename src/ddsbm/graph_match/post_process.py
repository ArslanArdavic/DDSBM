...
def SB_post_process(cfg, dataset_dir, perm_dir):
    ...
    dense_data_dict = torch.load(osp.join(dataset_dir, "raw/preprocessed.pt"), weights_only=False)
...
def post_process(cfg, dataset_dir, perm_dir):
    ...
    if cfg.graph_match.initialize:
        pyg_data_dict = torch.load(osp.join(dataset_dir, "train_data.pt"), weights_only=False)
